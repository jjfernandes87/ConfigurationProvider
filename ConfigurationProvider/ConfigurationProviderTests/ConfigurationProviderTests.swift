...
extension Bundle {
    ...
    class func componentsBundle(for componentClass: AnyClass, resource: String = "ConfigurationProvider") -> Bundle {
        if  let bundleURL = Bundle(for: componentClass).url(forResource: resource, withExtension: "bundle"),
            let bundle = Bundle(url: bundleURL)  {
            return bundle
        }
        if let bundle = Bundle(identifier: "br.com.mobile.ConfigurationProvider") {
            return bundle
        }
